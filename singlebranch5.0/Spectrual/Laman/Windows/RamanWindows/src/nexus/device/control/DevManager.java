...
//                ((AbstractTableModel) DeviceInfo.getModel()).fireTableDataChanged();
    ...
    SPDevManager devmanager = SpectralPlatService.GetInstance().GetDevManager();
    ...
    private void InitDeviceList() {
        ...
        this.List_DeviceName.setCellRenderer(new DefaultListCellRenderer() {
            ...
            public Component getListCellRendererComponent(JList list,
                ...
                java.net.URL disurl;
                /* 如果设备有控制器在使用，更新链接图片 */
                SSPDevInfo tmpdevinfo = devmanager.GetDeviceList().get(index).GetDevInfo();
                if (tmpdevinfo.SameAs(devmanager.GetDevControl().GetDevConnectInfo())) {
        ...
        this.List_DeviceName.setModel(new AbstractListModel() {
            ArrayList<ISPDevDriver> devList;

            {
                devList = SpectralPlatService.GetInstance().GetDevManager().GetDeviceList();
            }
        ...
        //注册设备列表刷新事件
        SpectralPlatService.GetInstance().GetDevManager().EventPod.RegeditListener(new EventListener() {
            @Override
        ...
        //注册设备控制状态变化事件
        this.devmanager.GetDevControl().RegisterStateChange(new EventListener() {
            @Override
            public void recevieEvent(Event event) {
                //更新设备界面
                SwingUtilities.invokeLater(new Runnable() {
    ...
    private void Button_DetectActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_Button_DetectActionPerformed
        ...
        threadpools.submit(new Runnable() {
            ...
            public void run() {
                ...
                devmanager.StartSearchIO(IOManager.CreateAllIO());
                if (devmanager.GetDeviceList().size() > 0) {
                    devmanager.GetDevControl().Open(devmanager.GetDeviceList().get(0));
                } else {
    ...
    private void List_DeviceNameMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_List_DeviceNameMouseClicked
        ...
        if (evt.getClickCount() == 2) {
            try {
                ...
                devmanager.GetDevControl().Open(devmanager.GetDeviceList().get(setlectIndex));
            } catch (Exception ex) {
