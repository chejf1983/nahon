...
import sps.dev.drv.SPDevSearch;
...
public class SPDevManager {
    ...
    public ArrayList<ISPDevice> GetDeviceList() {
        return this.devlist;
    }
    ...
    public void StartSearchIO(AbstractIO[] iolist) {
        ...
        for (AbstractIO io : iolist) {
            for (ISPDevice drv : SearchNDevs(io, (byte) 0, (byte) 0, 300)) {
    ...
    //搜索一个设备
    private ISPDevice[] SearchNDevs(AbstractIO io, byte minaddr, byte maxaddr, int timeout) {
        ArrayList<ISPDevice> result = new ArrayList();
        if (minaddr < 0 || minaddr > maxaddr) {
            //参数异常，最小值不能小于0，最小值也必须小于最大值
            return result.toArray(new ISPDevice[0]);
        }
        ...
        for (byte i = minaddr; i <= maxaddr; i++) {
            //跳过广播地址
            if (i != 0) {
                ISPDevice tmpdev = new SPDevSearch().SearchDevice(io, (byte) 0xFE, (byte) i, timeout);
                if (tmpdev != null) {
                    result.add(tmpdev);
                }
            }
        ...
        if (minaddr == 0 && result.isEmpty()) {
            ISPDevice tmpdev = new SPDevSearch().SearchDevice(io, (byte) 0xFE, (byte) 0, timeout);
            if (tmpdev != null) {
                result.add(tmpdev);
            }
        }
