...
public class DevControlActivity extends AppCompatActivity {
    ...
    private void init_dev_control() {
        ...
        //注册数据刷新接口
        this.common_app.RegisterDataCollectListener(new EventListener<SSpectralDataPacket>() {
            @Override
    ...
    private void setAllButtonEnable(boolean value) {
        ...
        if (!value) {
            ...
            this.button_stopcollect.setEnabled(state == ISPDevControl.CSTATE.BUSSY &&
                    SpectralPlatService.GetInstance().GetAppManager().GetCollectControl().IsStartCollect());
        }
    }
    // </editor-fold>
...
//        renderer.setShowGridY(true);    //显示纬线
    ...
    private CommonApp common_app = SpectralPlatService.GetInstance().GetAppManager().GetCommonApp();
